...
export const TopRatedCarousel = memo(function TopRatedCarousel() {
    ...
    const fetchTopRated = async () => {
        setLoading(true);
        try {
            const { data } = await rankingApi.getTopRated({ limit: 25 });
            ...
            console.error("Failed to fetch top rated single-genre books", error);
        } finally {
            setLoading(false);
        }
    };
